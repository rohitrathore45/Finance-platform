                ...
                <CardTitle>Monthly Budget (Default Account)</CardTitle>
                    <div className='flex items-center gap-2 mt-1'> 
                        {isEditing ? (<div className='flex items-center gap-2 '>
                            <Input type='number' disable={isLoading} value={newBudget} onChange={(e) => setNewBudget(e.target.value)} className='w-32' placeholder='Enter amount' autoFocus/>
                            <Button variant='ghost' disable={isLoading} size='icon' onClick={handleUpdateBudget}><Check className='h-4 w-4 text-green-500'/></Button>
                            <Button variant='ghost' disable={isLoading} size='icon' onClick={handleCancel}><X className='h-4 w-4 text-red-500'/></Button>
                        </div>) : (
                        <>
                            <CardDescription>
